kde.distribution <-
function( kernel, x, xi, hi,
                              lbound = FALSE, rbound = FALSE )
{

  H <- length(hi)      # number of bandwidth regions used
  N <- length(xi)      # number of observations used in the estimation
  m <- length(x)       # number of evaluation points in the domain
    ...
    for ( j in 1:m ) {
      ...
      if ( abs(u) < cutoff ) {
        ...
        if ( lbound == TRUE & x[j] < xleft ) {
          ...
          fx[j] <- fx[j] + kernel(u) / h  # interior domain point
